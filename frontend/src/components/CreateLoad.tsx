    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto m-4">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
          <h2 className="text-2xl font-semibold text-gray-900">Create New Load</h2>
          <button
            onClick={onClose}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex flex-col">
                        <span className={`px-3 py-1 text-xs font-medium rounded ${
                          ...
                          {isFirstStop ? 'First Stop - Pickup' : 
                           isLastStop ? 'Last Stop - Delivery' :
                           stop.type.charAt(0).toUpperCase() + stop.type.slice(1)}
                        </span>
                        {(isFirstStop || isLastStop) && (
                          <span className="text-xs text-gray-500 mt-1">
                            {isFirstStop ? '(Fixed)' : '(Fixed)'}
                          </span>
                        )}
                      </div>
                      {isIntermediate && (
                        <div className="flex flex-col">
                          <label className="text-xs text-gray-600 mb-1">Stop Type:</label>
                          <select
                            value={stop.type}
